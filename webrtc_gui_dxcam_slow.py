...
try:
    ...
    def _encode_frame_hw(self, frame, force_keyframe):
        ...
        if self.codec is None:
            ...
            # basic codec settings – constant parameters for stability
            self.codec.width = adj_w
            self.codec.height = adj_h
            # Constant-bitrate 4 Mbps; small GOP (60) and no B-frames for low latency
        ...
        try:
            ...
            print(f"[ERROR] Encoder {self.codec.name} failed: {encode_err}; skipping frame")
            # mark encoder as failed so next time we recreate (skip HW next round)
...
# ---------------------------------------------------------------------------
# Optimized Screen Capture Track
class ScreenTrack(VideoStreamTrack):
    """A video stream track that captures the local screen (optimized for 30 FPS)."""

    def __init__(self, send_fps: int = 30, scale: float = 0.5):
        super().__init__()
        self.send_fps = send_fps
        self.target_interval = 1.0 / send_fps
        self.scale = scale
        ...
        # Use DXGI Desktop Duplication on Windows for better performance
        import platform
        ...
        if platform.system() == "Windows":
            try:
                ...
                self.camera.start(target_fps=send_fps)  # Match capture to send FPS
                self.use_dxcam = True
        ...
        try:
            if cv2.cuda.getCudaEnabledDeviceCount() > 0:
                ...
                # removed invalid resizer init – we'll call cv2.cuda.resize directly
    ...
    async def recv(self):
        ...
        if self.use_dxcam:
            ...
            if frame_np is None:
                # No new frame yet - skip processing
                await asyncio.sleep(0)
                return await self.recv()
        else:
            # Use MSS for cross-platform support
        ...
        if frame_np.shape[2] == 4:
            if self.use_gpu:
                ...
                bgr_frame = cv2.cuda.cvtColor(self.gpu_frame, cv2.COLOR_BGRA2BGR)
            else:
                bgr_frame = cv2.cvtColor(frame_np, cv2.COLOR_BGRA2BGR)
        else:
            bgr_frame = frame_np
        ...
        if self.scale != 1.0:
            if self.use_gpu:
                resized_gpu = cv2.cuda.resize(bgr_frame, self.out_size)
                frame_np = resized_gpu.download()
            else:
                frame_np = cv2.resize(bgr_frame, self.out_size, interpolation=cv2.INTER_AREA)
        else:
            frame_np = bgr_frame

        # Convert to I420 (YUV420 planar)
        yuv = cv2.cvtColor(frame_np, cv2.COLOR_BGR2YUV_I420)
...
async def run_host(settings: Settings, creds: Credentials, status_cb: Callable[[str], None]):
    ...
    async with ClientSession() as session:
        async with session.ws_connect(settings.signalling_url) as ws:
            ...
            # Wait for authentication OK, then for viewer_waiting before we create offer
            authenticated = False
            async for msg in ws:
                if msg.type != WSMsgType.TEXT:
                    continue
                payload = msg.json()
                if not authenticated and payload.get("type") == "auth_result":
                    if not payload.get("success", False):
                        status_cb("Auth failed")
                        return
                    authenticated = True
                    status_cb("Authenticated – waiting for viewer…")
                    continue  # Wait for viewer_waiting next
                if authenticated and payload.get("type") == "viewer_waiting":
                    status_cb("Viewer ready – negotiating…")
                    break  # proceed to create offer
            ...
            video_track = ScreenTrack(send_fps=settings.fps, scale=settings.scale)
            ...
            async def _log_final_codec(sender: RTCRtpSender):
                ...
                if hasattr(enc, 'codec_context'):
                    ...
                    print(f"Final encoder if: {ctx.name}  ({ctx.width}x{ctx.height}, br={ctx.bit_rate})")
                else:
...
async def run_client(settings: Settings, creds: Credentials, status_cb: Callable[[str], None]):
    ...
    async with ClientSession() as session:
        async with session.ws_connect(settings.signalling_url) as ws:
            # Use a distinct viewer ID derived from host ID so signalling server treats connections separately
            viewer_id = f"{creds.conn_id}_viewer"
            # Auth as viewer
            await ws.send_json({"type": "auth", "id": viewer_id, "password": creds.password})
            # Notify host that this viewer is waiting
            await ws.send_json({"type": "viewer_waiting", "id": viewer_id})
